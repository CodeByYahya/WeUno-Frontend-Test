...
const Navbar = () => {
  ...
  return (
    <>
      <nav
        className={`${isOpen ? "fixed" : "absolute"} h-screen top-0 left-0 z-50 w-full ${
          isOpen ? "bg-custom-gradient" : "bg-transparent"
        }`}
      >
        ...
        <div className="hidden md:flex justify-center border-y-[1px] py-3 bg-transparent">
        <div className=" flex justify-around w-full">
          {mobileLinks.map((heading, index) => (
            <h3
              key={index}
              className="text-white text-md font-light hover:underline cursor-pointer"
            >
              {heading.text}
            </h3>
          ))}
